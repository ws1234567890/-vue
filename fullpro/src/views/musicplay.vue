<template>
  <div id="mu_play" @click="pasplay">
    <img class="bc" v-if="musicobj" :src="musicobj.al.picUrl" />
    <div class="mu_logo">
...
c4.271-0.012,7.729-3.47,7.74-7.74V7.74C33.989,3.47,30.53,0.011,26.26,0"
    ...
    <img src="../assets/needle-ip6.png" class="needle" />

    <div class="center">
      <div class="disc">
        <img class="circle" src="../assets/disc.png" />
        <img
          class="pic"
          v-if="musicobj"
          :src="musicobj.al.picUrl"
          :style="{'animation-play-state':isplay?'running':'paused'}"
        />
        <img class="btn" src="../assets/button.png" v-show="!isplay" @click.stop="muplay" />
      </div>
    </div>

    <!-- 音乐audio地址 -->
    <audio ref="audio" :src="audurl"></audio>
    <h2 class="title" v-if="musicobj">
      {{musicobj.name}} -
      <em
        v-for="(item,index) in musicobj.ar"
        :key="index"
        v-text="index>0?` / ${item.name}`:item.name"
      ></em>
    </h2>
  </div>
</template>
...
export default {
  ...
  created() {
    let id = this.$route.query.idd;
    api(`/song/detail?ids=${id}`).then(res => {
      this.musicobj = res.songs[0];
      this.audurl = `https://music.163.com/song/media/outer/url?id=${id}.mp3`;
    });
  },
  methods: {
    muplay() {
